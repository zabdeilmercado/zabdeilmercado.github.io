...
The dotted portrait is sampled into Canvas particles in `script.js`. Pointer movement repels nearby dots, while spring forces and damping return each dot to its original position. The “Scatter portrait” control applies a larger outward impulse and also gives touch and keyboard users a way to trigger the effect. Animation pauses when settled or outside the viewport, and the static image remains as a fallback when Canvas is unavailable or reduced motion is enabled.
